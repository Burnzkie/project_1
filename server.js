...
app.get('/login/Login.html', (req, res) => res.sendFile(path.join(__dirname, 'login', 'Login.html')));

// Serve dashboard
app.get('/dashboard', isAuthenticated, (req, res) => {
    res.sendFile(path.join(__dirname, 'dashboard', 'dashboard/index.html'));
});
...
app.get('/api/profile', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        if (results[0].profilePic) results[0].profilePic = `http://localhost:3000${results[0].profilePic}`;
        res.json(results[0]);
    } catch (error) {
...
// API endpoint to update profile (support profilePic deletion)
app.put('/api/profile', isAuthenticated, async (req, res) => {
    const { name, dob, profilePic } = req.body;
    if (!name) return res.status(400).json({ error: 'Name is required' });
    const { username } = req.session.user;
    try {
        const [user] = await db.query('SELECT profile_picture FROM user_info WHERE username = ?', [username]);
        if (profilePic === null && user[0]?.profile_picture) {
            const oldFilePath = path.join(__dirname, user[0].profile_picture.replace('/uploads/', 'Uploads/'));
            if (fs.existsSync(oldFilePath)) fs.unlinkSync(oldFilePath);
            await db.query('UPDATE user_info SET firstname = ?, dob = ?, profile_picture = NULL WHERE username = ?', [name, dob || null, username]);
        } else {
...
app.get('/api/audit-log/:id', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        if (results.length === 0) return res.status(404).json({ error: 'Log not found' });
        res.json(results[0]);
    } catch (error) {
...
app.get('/api/payment-plan/:id', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        if (results.length === 0) return res.status(404).json({ error: 'Payment plan not found' });
        res.json(results[0]);
    } catch (error) {
...
app.get('/api/tuition-fee/:id', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        if (results.length === 0) return res.status(404).json({ error: 'Tuition fee not found' });
        res.json(results[0]);
    } catch (error) {
...
app.post('/signup', async (req, res) => {
    const { firstname, lastname, email, gender, phone, country, region, city, brgy, street, username, password, role, dob } = req.body;
    try {
        const errors = [];
        ...
        const [result] = await db.query(
            'INSERT INTO user_info (firstname, lastname, email, gender, phone, country, region, city, brgy, street, username, password, role, dob) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [firstname, lastname, email, gender, phone, country, region, city, brgy, street, username, hashedPassword, role, dob || null]
        );
        res.json({ message: 'Signup successful' });
    } catch (error) {
        console.error('Error during signup:', error);
        if (error.code === 'ER_DUP_ENTRY') return res.status(400).json({ error: 'Email or username already exists.' });
        if (error.code === 'ER_NO_SUCH_TABLE') return res.status(500).json({ error: 'Database table "user_info" does not exist.' });
        if (error.code === 'ER_BAD_DB_ERROR') return res.status(500).json({ error: 'Database "shadow_1" does not exist.' });
